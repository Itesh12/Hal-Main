...
class SearchUserNotifier extends ChangeNotifier {
  SearchUserNotifier(this.ref, {required this.searchQuery}) {
    _initState();
  }
  ...
  List<SearchUserModel> get searchUsers => _searchUsers;
  ...
  Future<void> _firstLoad() async {
    ...
    try {
      _searchUsers = await ref
          .read(searchUserRepoProvider)
          .searchUser(page: _page, searchQuery: searchQuery);
      if (_searchUsers.length < 20) {
        _hasNextPage = false;
      }
      notifyListeners();
  ...
  //   _searchUsers.insert(0, comment);
  //   notifyListeners();
  // }

  // void removeUser(int index) {
  //   _searchUsers.removeAt(index);
  //   notifyListeners();
  // }
