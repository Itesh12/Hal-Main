...
class BazarAllProductListNotifier extends ChangeNotifier {
  BazarAllProductListNotifier(
    this.ref,
  ) {
    _initState();
  }
  ...
  List<GetBazarProductModel> _allBazarPost = [];

  List<GetBazarProductModel> get allBazarPost => _allBazarPost;
  ...
  Future<void> _firstLoad() async {
    ...
    notifyListeners();
    try {
      debugPrint("inside try, getting products");
      _allBazarPost = await ref.read(getMarketProductProvider).getBazarProducts(
          trade: 'all', userId: UserPreferences.userId, page: _page);
      debugPrint(" got the products");
      debugPrint(_allBazarPost.toString());

      if (_allBazarPost.length < 20) {
        _hasNextPage = false;
        debugPrint('no more products');
      }

      notifyListeners();
      debugPrint("notified the listener");
  ...
  Future<void> _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        final allBazarPost = await ref
            .read(getMarketProductProvider)
            .getBazarProducts(
                trade: 'all', userId: UserPreferences.userId, page: _page);
        if (allBazarPost.isNotEmpty) {
          _allBazarPost.addAll(allBazarPost);
          notifyListeners();
        } else {
