...
import 'package:kisaan_station/src/model/station/seller_mylisting_model.dart';

import '../../infrastructure/http/app_exception.dart';

class SellerListinNotifier extends ChangeNotifier {
  SellerListinNotifier(
    this.ref, {
  ...
  List<SellerMylistingModel> _listing = [];

  List<SellerMylistingModel> get listing => _listing;
  ...
  Future<void> _firstLoad() async {
    ...
    try {
      ...
      _listing = await ref
          ...
          .getSellerList(page: _page, sellerId: val.id, size: size);
      if (_listing.length < 20) {
        _hasNextPage = false;
      }
      notifyListeners();
  ...
  void deletePrd(String prdId) {
    _listing.removeWhere((element) => element.id == prdId);
    notifyListeners();
  }
