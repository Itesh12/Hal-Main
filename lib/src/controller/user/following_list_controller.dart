...
class FollowingListNotifier extends ChangeNotifier {
  ...
  List<FollowingListModel> _following = [];

  List<FollowingListModel> get following => _following;
  ...
  Future<void> _firstLoad() async {
    ...
    notifyListeners();
    try {
      _following = await ref
          .read(userRepoProvider)
          .getFollowingList(page: _page, stalkerId: _stalkerId, userId: userId);
      if (_following.length < 20) {
        _hasNextPage = false;
      }
      notifyListeners();
  ...
  Future<void> _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        final attend = await ref.read(userRepoProvider).getFollowingList(
